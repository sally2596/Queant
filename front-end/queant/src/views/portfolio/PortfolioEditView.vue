<template>
  ...
  <div>
    <div class="d-flex justify-content-center my-5">
      ...
      />
      <button class="searching" @click="fetchProductsByText(text)">
    ...
    <div class="container">
      ...
      <table class="rwd-table">
        <tbody>
          ...
          <tr v-for="myProduct in portfolio" :key="myProduct">
            <td data-th="Supplier Code">
              <router-link
                ...
                :to="{
                  name: 'bankInfoDetail',
                  params: { bankId: myProduct.product.bank_id },
                }"
              >
                <img
                  :src="myProduct.product.picture"
                  :alt="`${myProduct.product.name}`"
                />
                <!-- <div
            ...
            <td class="text-center" data-th="Supplier Name">
              <router-link
                ...
                :to="{
                  name: 'productDetail',
                  params: { productId: myProduct.product_id },
                }"
              >
            ...
            <td class="text-center" data-th="Invoice Number">
              {{
                String(myProduct.amount).replace(/\B(?=(\d{3})+(?!\d))/g, ",")
              }}원
            </td>
            ...
            <td
              class="text-center"
              data-th="Due Date"
              v-if="myProduct.product.deposit"
            >
              예금
            </td>
            <td
            ...
            <td class="text-center" data-th="Net Amount">
              <applied-rate :productForRate="myProduct"> </applied-rate>
            </td>
            <td>
              <button
                class="btn btn-outline-success btn-sm mx-1"
                @click="openMyProductModal(myProduct)"
              >
                수정
              </button>
              <button
                class="btn btn-outline-danger btn-sm mx-1"
                @click="deletePortfolio(myProduct.portfolio_id)"
              >
                삭제
              </button>
            </td>
      ...
      <table class="rwd-table">
        <tbody>
          ...
          <tr v-for="customProduct in customProducts" :key="customProduct">
            ...
            <td class="text-center" data-th="Invoice Number">
              {{
                String(customProduct.amount).replace(
                  /\B(?=(\d{3})+(?!\d))/g,
                  ","
                )
              }}원
            ...
            <td>
              <button
                class="btn btn-outline-success btn-sm mx-1"
                @click="openCustomProductEditModal(customProduct)"
              >
                수정
              </button>
              <button
                class="btn btn-outline-danger btn-sm mx-1"
                @click="deleteCustomProduct(customProduct.product_id)"
              >
                삭제
              </button>
            </td>
...
export default {
  ...
  methods: {
    ...
    openMyProductModal(payload) {
      (this.modalData = payload), (this.showMyProductModal = true);
    },
